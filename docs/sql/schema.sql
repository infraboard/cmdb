CREATE TABLE `bill` (
  ...
  KEY `idx_instance_id` (`instance_id`) USING HASH,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='资源月度账单'

CREATE TABLE `share_unit` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '单元Id',
  `type` tinyint(4) NOT NULL COMMENT '标签类型',
  `key` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '标签健',
  `value` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '标签值',
  `describe` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '标签描述',
  `weight` int(11) NOT NULL COMMENT '标签权重',
  `real_cost` float(10,2) NOT NULL COMMENT '分摊金额',
  `instance_id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '资源Id',
  `year` int(11) NOT NULL COMMENT '账单年份',
  `month` int(11) NOT NULL COMMENT '账单月份',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_id` (`key`,`value`,`instance_id`,`year`,`month`) COMMENT '实例月账单对于Tag分摊'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='共享资源成本分摊'

CREATE TABLE `cost_unit` (
  `id` int(11) NOT NULL,
  `type` tinyint(4) NOT NULL COMMENT '成本单元类型',
  `domain` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '成本单元所属域',
  `name` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '成本单元名称',
  `description` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '成本单元描述',
  `year` int(11) NOT NULL COMMENT '账单年份',
  `month` int(11) NOT NULL COMMENT '账单月份',
  `real_cost` float(10,2) NOT NULL COMMENT '账单金额',
  `delta_cost` float(10,2) NOT NULL COMMENT '同步增长金额',
  `delta_percent` float(255,0) DEFAULT NULL COMMENT '同步增长比例',
  PRIMARY KEY (`id`),
...
CREATE TABLE `resource` (
  `id` char(64) CHARACTER SET latin1 NOT NULL COMMENT '全局唯一Id, 直接使用个云商自己的Id',
  `resource_type` tinyint(2) NOT NULL COMMENT '资源类型',
  `vendor` tinyint(1) NOT NULL COMMENT '厂商',
  `region` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '地域',
  `zone` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '区域',
  `create_at` bigint(13) NOT NULL COMMENT '创建时间',
  `expire_at` bigint(13) DEFAULT NULL COMMENT '过期时间',
  `category` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '种类',
  `type` varchar(120) CHARACTER SET latin1 NOT NULL COMMENT '规格',
  `name` varchar(255) NOT NULL COMMENT '名称',
  `description` varchar(255) DEFAULT NULL COMMENT '描述',
  `status` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '服务商中的状态',
  `update_at` bigint(13) DEFAULT NULL COMMENT '更新时间',
  `sync_at` bigint(13) DEFAULT NULL COMMENT '同步时间',
  `sync_accout` varchar(255) CHARACTER SET latin1 DEFAULT NULL COMMENT '同步账号',
  `public_ip` varchar(64) CHARACTER SET latin1 DEFAULT NULL COMMENT '公网IP',
  `private_ip` varchar(64) CHARACTER SET latin1 DEFAULT NULL COMMENT '内网IP',
  `pay_type` varchar(255) CHARACTER SET latin1 DEFAULT NULL COMMENT '实例付费方式',
...
CREATE TABLE `rds` (
  `engine_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '引擎 比如 MYSQL, SQLServer, PGSQL',
  `engine_version` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '引擎版本',
  `instance_class` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例规格: 对应ALI(DBInstanceClass)',
  `class_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例规格族,取值: s: 共享型; x: 通用型; d: 独享套餐; h: 独占物理机',
  `export_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例是内网或外网 (Internet: 外网/Intranet: 内网)',
  `network_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例的网络类型 (Classic: 经典网络/VPC: 专有网络。)',
  `type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例类型 Primary: 主实例, Readonly: 只读实例, Guard: 灾备实例, Temp: 临时实例',
  ...
  `collation` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '系统字符集排序规则',
  `time_zone` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '时区',
  `storage_capacity` int(11) NOT NULL COMMENT '实例存储空间,单位: GB。',
  `storage_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例储存类型 local_ssd/ephemeral_ssd: 本地SSD盘, cloud_ssd: SSD云盘; cloud_essd: ESSD云盘',
  `security_ip_mode` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '安全名单模式, 默认白名单',
  `security_ip_list` text CHARACTER SET latin1 NOT NULL COMMENT 'IP白名单',
  `connection_mode` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例的访问模式,取值: Standard: 标准访问模式; Safe: 数据库代理模式。',
  `ip_type` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT 'IP类型',
  `lock_mode` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '实例锁定模式; Unlock: 正常; ManualLock: 手动触发锁定; LockByExpiration: 实例过期自动锁定; LockByRestoration: 实例回滚前的自动锁定; LockByDiskQuota: 实例空间满自动锁定',
  `lock_reason` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '锁定原因',
  `deploy_mode` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '部署模式(腾讯云独有)',
  `port` int(11) NOT NULL COMMENT '端口',
  `extra` text CHARACTER SET latin1 NOT NULL COMMENT '额外的无法通用的一些属性, 比如只有腾讯云独有的一些属性'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='rds实例信息'

CREATE TABLE `secret` (
  `id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '凭证Id',
  `create_at` bigint(13) NOT NULL COMMENT '创建时间',
  `description` varchar(255) NOT NULL COMMENT '凭证描述',
  `vendor` tinyint(1) NOT NULL COMMENT '资源提供商',
  `address` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '体验提供方访问地址',
  `allow_regions` text CHARACTER SET latin1 NOT NULL COMMENT '允许同步的Region列表',
  `crendential_type` tinyint(1) NOT NULL COMMENT '凭证类型',
  `api_key` varchar(255) NOT NULL COMMENT '凭证key',
  `api_secret` text CHARACTER SET latin1 NOT NULL COMMENT '凭证secret',
  `request_rate` int(11) NOT NULL COMMENT '请求速率',
  `domain` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '所属域',
  `namespace` varchar(255) CHARACTER SET latin1 NOT NULL COMMENT '所属空间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_key` (`api_key`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='资源提供商同步凭证管理'

CREATE TABLE `task` (
  `id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '任务Id',
  `region` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '资源所属Region',
  `resource_type` tinyint(1) NOT NULL COMMENT '资源类型',
  `secret_id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '用于操作资源的凭证Id',
  `secret_desc` text NOT NULL COMMENT '凭证描述',
  `timeout` int(11) NOT NULL COMMENT '任务超时时间',
  `status` tinyint(1) NOT NULL COMMENT '任务当前状态',
  `message` text CHARACTER SET latin1 NOT NULL COMMENT '任务失败相关信息',
...
CREATE TABLE `task_record` (
  `instance_id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT '实例Id',
  `instance_name` varchar(255) NOT NULL COMMENT '实例名称',
  ...
  `task_id` varchar(64) CHARACTER SET latin1 NOT NULL COMMENT 'task id',
